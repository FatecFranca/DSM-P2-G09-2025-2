...
function selecionarVeiculoParaChecklist(id, placa, donoNome) {
  ...
  const inputVeiculo = document.getElementById('checkVeiculoId');
  if (inputVeiculo) inputVeiculo.value = id;
...
async function enviarChecklist(e) {
  ...
  const fk_id_veiculo = document.getElementById('checkVeiculoId')?.value || null;
  ...
  if (!fk_id_veiculo) {
    return alert('Por favor selecione um veículo antes de salvar o checklist.');
  }
  const itens = [];
  const checkboxes = [
  ...
  const payload = {
    ...
    fk_id_veiculo: fk_id_veiculo ? Number(fk_id_veiculo) : null,
    criado_em: nowISO
  };
  ...
  if (document.getElementById('checkVeiculoId')) document.getElementById('checkVeiculoId').value = '';
  const previewImgs = document.querySelectorAll('#previewChecklistImagens img.preview-thumb');
  if (previewImgs.length > 0) {
    previewImgs.forEach(img => {
